...
import { timelineLayout, extraTimelineHeight } from "./timeline-layout";
...
const DATA_URL = `${import.meta.env.BASE_URL}portfolio.json`;

async function start() {
  const app = document.querySelector<HTMLDivElement>("#app")!;
  app.innerHTML = `<div class="data-status" role="status">${spark}<p>Loading the journey...</p></div>`;
  try {
    const data = await loadPortfolio(DATA_URL);
    renderPortfolio(data);
  } catch (error) {
    app.innerHTML = `<main class="data-status"><div>${spark}<h1>The story is taking a moment.</h1><p>Check <code>public/portfolio.json</code>, then try again.</p><p class="data-error" role="alert"></p><button class="button button-primary" type="button">Try again ${icon("arrow")}</button></div></main>`;
    app.querySelector(".data-error")!.textContent =
      error instanceof Error
        ? error.message
        : "The portfolio could not be loaded.";
    app.querySelector("button")!.addEventListener("click", () => void start());
  }
}

function renderPortfolio(data: PortfolioData) {
  const { profile, milestones, hero, work, about, contact } = data;
  const email = profile.email;
  const emailHref = `mailto:${email.split("@").map(encodeURIComponent).join("@")}`;
  const e = escapeHtml;
  const personalize = (value: string) =>
    value.replaceAll("{name}", profile.name);
  const f = (value: string) => formatText(personalize(value));
  const countLabel = String(milestones.length).padStart(2, "0");
  const anchors = timelineLayout(milestones.length);
  document.title = personalize(data.site.title);
  document
    .querySelector('meta[name="description"]')
    ?.setAttribute("content", personalize(data.site.description));
  ...
  const artwork = { orbit: orbitArt, forma: formaArt, still: stillArt };
  const projects = data.projects.map((project, index) => ({
    ...project,
    art: project.image
      ? `<img class="project-art project-photo" src="${e(new URL(project.image, new URL(DATA_URL, window.location.href)).href)}" alt="" loading="lazy" decoding="async">`
      : artwork[project.artwork].replaceAll(
          "chart-fill",
          `chart-fill-${index}`,
        ),
  }));
  ...
  <header class="site-header">
    <a class="brand" href="#journey" aria-label="${e(profile.name)}, back to the journey">${spark}<span class="brand-type">${e(profile.name.toUpperCase())}<span>${e(profile.role.toUpperCase())}</span></span></a>
  ...
  <main id="main">
    <section class="hero" id="journey" aria-labelledby="hero-title" style="--timeline-extra:${extraTimelineHeight(milestones.length)}px">
      ...
      <div class="hero-intro">
        <p class="eyebrow"><span class="little-line"></span>${e(hero.eyebrow)}</p>
        <h1 id="hero-title">${f(hero.title)}</h1>
        <p class="hero-description">${f(hero.description)}</p>
        ...
        <div class="intro-note"><span class="availability-dot"></span>${e(profile.availability)}</div>
      </div>
      <div class="scene-stage" role="group" aria-label="${e(milestones.length ? `Interactive career timeline from ${milestones[0].year} to ${milestones[milestones.length - 1].year}` : "Career timeline")}">
        <div class="scene-heading"><span></span>ONE JOURNEY. INFINITE POSSIBILITIES.</div>
        ${milestones.map((milestone, index) => `<button class="milestone ${index === milestones.length - 1 ? "milestone-current" : ""}" data-milestone="${index}" style="--fallback-x:${anchors[index].x * 100}%;--fallback-y:${anchors[index].y * 100}%" aria-label="${e(milestone.year)}: ${e(milestone.role)}. Read this chapter."><span class="milestone-top"><span class="milestone-year">${e(milestone.year)}</span>${index === milestones.length - 1 ? '<span class="now-badge">NOW</span>' : icon("diagonal", "milestone-arrow")}</span><span class="milestone-role">${e(milestone.role)}</span><span class="milestone-note">${e(milestone.note)}</span></button>`).join("")}
        ${data.alternatives.map((alternative, index) => `<div class="possibility ${index % 2 ? "possibility-two" : "possibility-one"}" style="top:${data.alternatives.length <= 2 ? (index % 2 ? 65 : 35) : 25 + (index / data.alternatives.length) * 55}%" aria-hidden="true"><span>${e(alternative.year)}</span>${e(alternative.label)}<small>${e(alternative.note)}</small></div>`).join("")}
        ${milestones.length ? "" : '<p class="timeline-empty">More chapters are on the way.</p>'}
    ...
    <section class="work-section content-section" id="work" aria-labelledby="work-title">
      <div class="section-kicker"><span>${e(work.eyebrow)}</span><span>${e(work.aside)}</span></div>
      <div class="section-heading"><h2 id="work-title">${f(work.title)}</h2><p>${f(work.description)}</p></div>
      <div class="project-grid">${projects.map((project, index) => `<button class="project-card" data-project="${index}" aria-label="Explore ${e(project.name)}: ${e(project.summary)}"><div class="project-image">${project.art}<span class="project-open">${icon("diagonal")}</span><span class="project-number">${String(index + 1).padStart(2, "0")}</span></div><div class="project-meta"><span>${e(project.category)}</span><span>${e(project.year)}</span></div><div class="project-title-row"><h3>${e(project.name)}</h3>${icon("diagonal")}</div><p>${e(project.summary)}</p></button>`).join("") || '<p class="empty-state">New projects are on the way.</p>'}</div>
      <div class="work-footnote"><span>${e(work.note)}</span><span>${e(work.closingNote)}</span></div>
    </section>

    <section class="about-section content-section" id="about" aria-labelledby="about-title">
      <div class="section-kicker"><span>${e(about.eyebrow)}</span><span>${e(about.aside)}</span></div>
      <div class="about-layout"><div class="about-visual" aria-hidden="true"><div class="about-orbit orbit-a"></div><div class="about-orbit orbit-b"></div><div class="about-orbit orbit-c"></div><div class="about-core">${spark}</div><span class="orbit-label label-design">DESIGN</span><span class="orbit-label label-code">CODE</span><span class="orbit-label label-curiosity">CURIOSITY</span><span class="about-coordinate">${e(profile.coordinates)}</span><span class="about-visual-note">A MIND AT THE INTERSECTION.</span></div><div class="about-copy"><p class="eyebrow">${e(about.intro)}</p><h2 id="about-title">${f(about.title)}</h2>${about.paragraphs.map((paragraph) => `<p>${f(paragraph)}</p>`).join("")}<div class="skill-list">${about.skills.map((skill) => `<span>${e(skill)}</span>`).join("")}</div><a class="text-link" href="#contact">${e(about.contactPrompt)} ${icon("diagonal")}</a></div></div>
    </section>

    <section class="contact-section content-section" id="contact" aria-labelledby="contact-title"><div class="contact-orbit" aria-hidden="true"></div><p class="eyebrow"><span class="availability-dot"></span>${e(contact.eyebrow)}</p><h2 id="contact-title">${f(contact.title)}</h2><p>${f(contact.description)}</p><button class="button button-primary" data-contact>Start a conversation ${icon("diagonal")}</button><a class="contact-email" href="${e(emailHref)}">${e(email)}</a></section>
  </main>
  <footer class="site-footer"><a class="footer-brand" href="#journey">${spark}<span>${e(profile.name.toUpperCase())}</span></a><p>${e(data.footer.note)}</p><div><span>&copy; ${new Date().getFullYear()}</span><a href="#journey" aria-label="Back to top">BACK TO TOP ${icon("diagonal")}</a></div></footer>
  <dialog class="road-dialog" id="road-dialog" aria-labelledby="road-title" aria-describedby="road-help">
    ...
    <div class="road-hud">
      <ol class="road-stations ${milestones.length > 8 ? "many-stations" : ""}" aria-label="Journey stops">${milestones.map((milestone, index) => `<li data-road-station="${index}"><span class="station-dot" aria-hidden="true"></span><span>${e(milestone.year)}</span></li>`).join("")}</ol>
      <div class="road-stop-info"><p class="road-stop-kicker">THE BEGINNING <span class="road-count">00 / ${countLabel}</span></p><h3 class="road-stop-title">It starts with curiosity.</h3><p class="road-stop-note">Take the road, one experience at a time.</p><button class="road-read text-link" type="button" hidden>Read this chapter ${icon("diagonal")}</button></div>
...
`;
  ...
  roadEntry.hidden = milestones.length === 0;
  const roadPrevious =
  ...
  function updateRoad(state: RoadSceneState) {
    roadState = state;
    roadEntry.disabled = !state.available || milestones.length === 0;
    ...
    roadDialog.querySelector(".road-stop-kicker")!.innerHTML =
      `${state.traveling ? "ON THE WAY" : milestone ? e(milestone.year) : "THE BEGINNING"}<span class="road-count">${String(index + 1).padStart(2, "0")} / ${countLabel}</span>`;
    ...
    roadNext.disabled = !state.available || milestones.length === 0;
    ...
    roadNext.querySelector("span")!.textContent = state.traveling
      ...
      : state.stopIndex === milestones.length - 1
        ...
        : state.stopIndex < 0
          ...
          : "Next stop";
    roadRead.hidden =
      state.stopIndex < 0 || state.traveling || !state.available;
    ...
    roadDialog.querySelector(".road-announcement")!.textContent =
      !state.available
  ...
  function showMilestone(index: number) {
    if (
      roadDialog.open &&
      (roadState.traveling || index !== roadState.stopIndex)
    )
      return;
    const milestone = milestones[index];
    ...
    openDialog(
      `<div class="chapter-year">${e(milestone.year)}<span>CHAPTER ${String(index + 1).padStart(2, "0")} / ${countLabel}</span></div><p class="eyebrow">${e(milestone.role)}</p><h2 id="dialog-title">${f(milestone.title)}</h2><p class="dialog-description">${f(milestone.description)}</p><blockquote>${f(milestone.reflection)}</blockquote><div class="skill-list">${milestone.skills.map((skill) => `<span>${e(skill)}</span>`).join("")}</div>${chapterNavigation}`,
      "milestone",
    );
    dialogContent
  ...
  document
    ...
    .forEach((button) => {
      button.addEventListener("click", () => {
        ...
        openDialog(
          `<div class="dialog-project-art">${project.art.replaceAll("chart-fill", "dialog-chart-fill")}</div><p class="eyebrow">${e(project.kind)} / ${e(project.year)}</p><h2 id="dialog-title">${e(project.name)}<span class="project-dialog-arrow">${icon("diagonal")}</span></h2><p class="dialog-subtitle">${e(project.summary)}</p><p class="dialog-description">${f(project.description)}</p><p class="dialog-description">${f(project.detail)}</p><div class="skill-list">${project.stack.map((skill) => `<span>${e(skill)}</span>`).join("")}</div><button class="text-link project-contact">Have something in mind? Let's talk ${icon("diagonal")}</button>`,
          "project",
        );
        dialogContent
  ...
  if (milestones.length === 0) {
    ...
    document.querySelector(".scene-instruction")?.remove();
  } else
    void import("./scene")
      .then(({ createJourneyScene }) => {
        ...
        roadEntry.disabled = milestones.length === 0;
      })
      .catch((error: unknown) => {
        console.warn(
        ...
        roadEntry.querySelector("span")!.textContent =
          "Ground view requires WebGL";
      });
